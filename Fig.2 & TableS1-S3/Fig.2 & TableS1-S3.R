...
df <- read_excel("Fig2 & S3 & TableS1-6.xlsx", sheet = "Fig3_data")[, 1:32]
...
# ------------------------------------------------------------------------------
# Function 2: Distribution of actual PSF (richness levels)
# ------------------------------------------------------------------------------
Actual_PSF_new <- function(n_perm = n_iter) {
  resampled <- list()
  for (Richness_con in levels_rich) {
    if (Richness_con == "1") {
...
actual_dist_new <- Actual_PSF_new()

# ------------------------------------------------------------------------------
# Function 3: Hypothesis testing and summary function
# ------------------------------------------------------------------------------
NHST_summary_new <- function(null_data, actual_data) {
  output <- list()
  for (Richness_con in levels_rich) {
    summary <- list()
    actual_vec <- actual_data[[Richness_con]]
    if (length(actual_vec) == 0 || all(is.na(actual_vec))) {
      summary[["Actual"]] <- c(NA, NA, NA, NA)
    } else {
      summary[["Actual"]] <- c(quantile(actual_vec, 0.025, na.rm = TRUE),
                               mean(actual_vec, na.rm = TRUE),
                               quantile(actual_vec, 0.975, na.rm = TRUE), NA)
    }
    for (type in c("Richness_hypothesis", "Biomass_ratio_hypothesis")) {
    ...
    summary_df$ES <- c("Actual", "Richness_hypothesis", "Biomass_ratio_hypothesis")
    output[[Richness_con]] <- summary_df
  }
  return(output)
}

nhst_new <- NHST_summary_new(null_dist_new, actual_dist_new)


# ------------------------------------------------------------------------------
# Step3: Table S4: Summary results
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Step4: Figure 3(b) – Comparison of actual and predicted PSF
# ------------------------------------------------------------------------------
b <- ggplot() +
  theme_bw() +
  coord_flip() + 
  scale_y_discrete(limits = levels_rich) +
  geom_estci(data = ES_plot_data_new[ES_plot_data_new$Model == "Actual", ], 
  ...
  mytheme +
  labs(x = "Community-level PSFs", y = "Species richness in conditioning community", tag = "(b)")
b
...
##################################################################################################################
#####                                                                                                        ##### 
#####                    Part 3---Calculate actual PSF and two predicted PSFs for each pot                   #####
#####                                                                                                        #####  
################################################################################################################## 
...
# ----------------------------------------------------------------------- 
# Step 2:  Actual PSF (pot-level)
# ----------------------------------------------------------------------- 
pots <- df[df$remark %in% levels_rich, ]
pots$Actual_PSF_Pot <- sapply(pots$TG_Pot_res, function(t) mean(log(t / CK_vals), na.rm = TRUE))
...
# -----------------------------------------------------------------------
pot_predictions <- pots[, c("Pot", "remark", "Richness_con", "TG_Pot_res", "Actual_PSF_Pot", "pred_RichnessPSF_Pot", "pred_BiomassRatioPSF_Pot")]
