...
mytheme= theme(legend.position = "none",
               ...
               legend.text = element_text(size = 8, lineheight = 1.33),  
...
#####                                       Fig.1                                     #####
###########################################################################################
setwd('D:/2025.10.4-NPH/0-Code-alive-1216/code-0108-R1/Fig.1')
  
data_li = read.xlsx("data_Fig.1.xlsx", sheet="bar" ,colNames = T, rowNames = F);head(data)

long_data <- data_li %>% pivot_longer(cols = -MaxNo._Cspp., names_to = "Year", values_to = "No._PSF_studies" ) %>% mutate(Year = as.numeric(Year))  
dim(long_data)
long_data$MaxNo._Cspp. = factor(long_data$MaxNo._Cspp.)
long_data$MaxNo._Cspp. = factor(long_data$MaxNo._Cspp., levels = c("1","2","3","4","5","6","8","9","10","12",">12"))

### bar
ggplot(long_data, aes(x = Year, y = No._PSF_studies, alluvium = MaxNo._Cspp., fill = MaxNo._Cspp. )) +  
...
ggsave("Fig.1.pdf", plot = p1,width = 17.3, height = 18 ,  units = "cm", dpi = 600)  
