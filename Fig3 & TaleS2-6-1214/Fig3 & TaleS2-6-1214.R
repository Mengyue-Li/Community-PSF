...
### Table S2
#--------------------------------------
data_sheet1 = read.xlsx("data_Fig3_1214.xlsx",sheet="homeaway"); data_sheet1[1:6,1:12]
...
### Table S3 &5
#--------------------------------------
data <- data_sheet1 %>% slice(-(1:5)); data[1:6,1:12]
...
#####                                                                        #####  
##################################################################################
df <- read_excel("data_Fig3_1214.xlsx",sheet="PSF", na = c("NA", "N/A"));head(df)
# Data processing note about df[, c("A", "B")] : Missing above-ground biomass (NA) for individual species 
# Since we only calculate relative biomass for each species ues this data(these values are used as weights in the weighted calculations)
# e.g.  Row 7: 8.63 / 8.63 = 1
#       Row 287, species H: 4.45 / (4.45 + 5.34)
#       Row 287, species H: 4.45 / (4.45 + 5.34) 
# Missing species(NA) should contribute 0 to both numerator and denominator
# To maintain consistency in the code operations, we set the missing (NA) values to 0.
df <- df %>%mutate(across(where(is.numeric), ~ ifelse(is.na(.), 0, .)));head(df)
...
a <- ggplot() +
  ...
  geom_rect(data = NULL, 
            aes(xmin = -Inf, xmax = Inf, 
                ymin = which(levels(df$remark) == "1") - 1, 
...
b <- ggplot() +
  ...
  geom_estci(data = ES_plot_data[ES_plot_data$Model == "Additive" & ES_plot_data$Lv %in% c("1", "2", "4", "6", "12"), ], 
             ...
             color = "#7B3D12", shape=15,size = 0.6, ci.linesize = 0.5, position = position_nudge(y = +0.1)) +
  geom_estci(data = ES_plot_data[ES_plot_data$Model == "Weighted_Additive" & ES_plot_data$Lv %in% c("1", "2", "4", "6", "12"), ], 
             aes(x = Mean, y = Lv, xmin = Low, xmax = High, xintercept = 0), 
             color = "#DE8125",shape=17, size = 0.6, ci.linesize = 0.5, position = position_nudge(y = +0.2)) +
...
Ln(biomass in conditioned soil 
/ biomass in sterilized)", y = " ", tag = "(b)") 
b
