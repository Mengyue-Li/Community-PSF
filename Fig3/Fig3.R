...
# F4, 279 = 0.32, P = 0.865
data$Richness_con = as.factor(data$Richness_con)
mod_01 <- aov(AG_con ~ Richness_con, data = data)
summary(mod_01)-> mod_01_result;mod_01_result 
...
#####                                                                        #####  
##################################################################################
df <- read_excel("data_Fig3.xlsx",sheet="PSF_raw");head(df)
      ...
      for (j in 1:nrow(combination)) {
        ...
        for (id in 1:n_perm) {
          ...
          joint_effect <- if (length(log_effects) == 0) {
            ...
            switch(
              type,#********************************* 
              "Additive" = sum(log_effects)/ length(log_effects),
...
ggsave("Fig.3-1208.pdf",plot = P,width = 12, height = 11, units = "cm",  dpi = 600)           
