...
###########################################################################################
#chooseCRANmirror()
#devtools::install_github('brendanf/FUNGuildR'c)
library(readxl)
library(openxlsx)
library(multcomp)
library(tidyverse)
...
# raw data
setwd("C:/Users/梦月/Desktop/CODE/Fig.2")
  
MetaTab <- read_excel("data_Fig2.xlsx",sheet=1);
MetaTab <- as.data.frame(MetaTab);rownames(MetaTab) <- MetaTab[,1] ;MetaTab <- MetaTab[MetaTab$Monoculture == "NO",]
Biomass_dat <- read_excel("data_Fig2.xlsx",sheet=2);Biomass_dat <- as.data.frame(Biomass_dat);rownames(Biomass_dat) <- Biomass_dat[,1] 
seqDat_full <- read_excel("data_Fig2.xlsx",sheet=3); dim(seqDat_full);seqDat_full <- as.data.frame(seqDat_full);rownames(seqDat_full) <- seqDat_full[,1]
tax <- seqDat_full$Taxonomy; tax <- as.data.frame(tax); rownames(tax) <- rownames(seqDat_full);seqDat <- seqDat_full[,c(2:281)]
...
set.seed(123)
seqDat_Flattening_pathogen <- as.data.frame(t(rrarefy(t(seqDat_plantPathogen), min(colSums(seqDat_plantPathogen)))))
...
set.seed(123)
seqDat_Flattening_AMF <- as.data.frame(t(rrarefy(t(seqDat_AMF), min(colSums(seqDat_AMF)))))
...
#----------------------Table S3-2: RDA 
### RDA_result_overall
normData <- read.xlsx("data_Fig2.xlsx", sheet =4, colNames = T, rowNames = T)
