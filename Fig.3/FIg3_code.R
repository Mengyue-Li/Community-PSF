...
#--------------------------------------------------------------------
# raw data
setwd("C:/Users/MY/Desktop/li/Fig.3")

MetaTab <- read_excel("data_Fig.3.xlsx",sheet=1);
MetaTab <- as.data.frame(MetaTab);rownames(MetaTab) <- MetaTab[,1] ;MetaTab <- MetaTab[MetaTab$Monoculture == "NO",]
Biomass_dat <- read_excel("data_Fig.3.xlsx",sheet=2);Biomass_dat <- as.data.frame(Biomass_dat);rownames(Biomass_dat) <- Biomass_dat[,1] 
seqDat_full <- read_excel("data_Fig.3.xlsx",sheet=3); seqDat_full <-seqDat_full[,1:282]; dim(seqDat_full);seqDat_full <- as.data.frame(seqDat_full);rownames(seqDat_full) <- seqDat_full[,1]
...
# or directly load the data
 taxMod_with_guilds <-read_excel("data_Fig.3.xlsx",sheet=3);taxMod_with_guilds<- as.data.frame(taxMod_with_guilds); rownames(taxMod_with_guilds) <- taxMod_with_guilds[,1] 
taxMod_all <- merge(taxMod_with_guilds,taxMod,by=0, all=TRUE);rownames(taxMod_all) <- taxMod_all[,1]
...
#----------------------Table S5-2: RDA 
### RDA_result_overall
normData <- read.xlsx("data_Fig.3.xlsx", sheet =4, colNames = T, rowNames = T)
